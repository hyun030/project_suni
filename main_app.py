    ...
    with tabs[4]:
        ...
        with col1:
            ...
            if st.button("📥 보고서 생성", type="primary", key="make_report"):
                financial_data = st.session_state.financial_data if st.session_state.financial_data is not None else st.session_state.manual_financial_data
                
                if financial_data is None:
                    st.error("보고서를 생성할 재무 데이터가 없습니다. 먼저 분석을 실행해주세요.")
                else:
                    try:
                        with st.spinner("보고서를 생성 중입니다..."):
                            if report_format == "PDF":
                                pdf_bytes = create_enhanced_pdf_report(financial_data)
                                st.download_button(
                                    label="📄 PDF 보고서 다운로드",
                                    data=pdf_bytes,
                                    file_name=f"SK에너지_경쟁사분석_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                                    mime="application/pdf"
                                )
                            else:
                                excel_bytes = create_excel_report(financial_data)
                                st.download_button(
                                    label="📊 Excel 보고서 다운로드",
                                    data=excel_bytes,
                                    file_name=f"SK에너지_경쟁사분석_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
